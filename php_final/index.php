...
?>
    <div id="board_area">
        ...
        <span id="mem_info">
            ...
            <?php echo $_SESSION['username']; ?>님 어서오세요. &nbsp;&nbsp;&nbsp;<a href="logout.php">로그아웃</a>
            &nbsp;&nbsp;&nbsp;<a href="user_modify.php">회원수정</a>&nbsp;&nbsp;&nbsp;<a href="user_answer.php">회원탈퇴</a>
            <?php
                if($_SESSION['userid'] == 'admin') {
                    echo '&nbsp;&nbsp;&nbsp;<a href="admin.php">관리자모드</a><br />';}
                else {
                    echo "<br>";
                }
                ?>
        ...
        if(isset($_GET['page'])){
         $page = $_GET['page'];
             ...
             $title=$board["title"]; 
               ...
               $con_idx = $board["idx"];
               $reply_count = "SELECT COUNT(*) as cnt FROM free_reply where con_num=$con_idx";
                ...
                <tr>
                    ...
                    <td width="500"><?php
                     ...
                     if($board['lock_post'] == "1") {
                        ?><?php echo $title, $lockimg, $img; 
                        if(isset($_SESSION['userid'])) {
                            echo "<a href='free_ck_read.php?idx=". $board["idx"]. "'>  [게시글읽기]</a>";
                        } else {
                            echo "  [로그인 후 글 읽기 가능]";
                        }
                        }else {?>
                        <?php echo $title,$img;
                        if(isset($_SESSION['userid'])) {
                            echo "<a href='free_read.php?idx=". $board["idx"]. "'>  [게시글읽기]</a>";
                        }
                        else {
                            echo "  [로그인 후 글 읽기 가능]";
                        }
...
if (isset($_SESSION['userid'])) {
    ...
    // 세션 정보가 없는 경우
    echo "<div id='write_btn'>
              <button onclick='alert(\"로그인 후에 글을 작성할 수 있습니다.\")'>글쓰기</button>
          </div>";
}
?>
