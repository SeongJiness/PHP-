...
<body>
    <?php
        ...
        ");
		 ...
		 /* bno함수에 idx값을 받아와 넣음*/
        $hit = "select * from free where idx ='".$bno."'";
        $hit = mysqli_query($con, $hit);
		$hit = mysqli_fetch_array($hit);
		$hit = $hit['hit'] + 1;
        $fet = "update free set hit = '".$hit."' where idx = '".$bno."'";
        $fet = mysqli_query($con, $fet);
        $sql = "select * from free where idx='".$bno."'";
    ...
    <div id="board_read">
        ...
        <div id="bo_ser">
            <ul>
                <li><a href="free.php">[목록으로]</a></li>
                <li><a href="free_thumbup.php?idx=<?php echo $board['idx']; ?>">[추천]</a></li>
                <?php
               if(isset($_SESSION['userid']) && $_SESSION['userid'] == $board['name']) {
                echo '<li><a href="free_modify.php?idx='. $board['idx']. '">[수정]</a></li>';
                echo '<li><a href="free_answer.php?idx='. $board['idx']. '">[삭제]</a></li>';
            }
                ?>
            </ul>
        </div>
    ...
    <div class="reply_view">
        <h3>댓글목록</h3>
        <?php
			$sql3 = "select * from free_reply where con_num='".$bno."' order by idx desc";
        ...
        <div class="dap_lo">
            ...
            <div class='dat_delete'>
                <form action="free_reply_delete.php" method="post">
                    <input type="hidden" name="rno" value="<?php echo $reply['idx']; ?>" />
                    <input type="hidden" name="b_no" value="<?php echo $bno; ?>">
        ...
        <!--- 댓글 입력 폼 -->
        <div class="dap_ins">
            <form action="free_reply.php?idx=<?php echo $bno; ?>" method="post">
