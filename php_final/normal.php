...
?>
    ...
    <div id="board_area">
        <h1>일반유저게시판</h1>
        <h4>일반유저이상 회원만 글을 볼 수 있는 게시판입니다.</h4>
        <span id="mem_info">
            <?php
...
if(isset($_SESSION['userid'])){ // 로그인한 사용자인 경우
        ...
        if(isset($_GET['page'])){
         $page = $_GET['page'];
             ...
             $sql2 = "select * from normal order by idx desc limit $start_num, $list";
             ...
             $title=$board["title"]; 
               ...
               $con_idx = $board["idx"];
               $reply_count = "SELECT COUNT(*) as cnt FROM normal_reply where con_num=$con_idx";
            ...
            <tbody>
                <tr>
                    ...
                    if($board['lock_post'] == "1") {
                        ?><a href="normal_ck_read.php?idx=<?php echo $board["idx"];?>"><?php echo $title, $lockimg, $img; 
                     }else {?>
                            <a href="normal_read.php?idx=<?php echo $board["idx"];?>"><?php echo $title,$img;}?></a>
                    </td>
