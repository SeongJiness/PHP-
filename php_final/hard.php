...
?>
    ...
    <div id="board_area">
        <h1>열심게시판</h1>
        <h4>열심등급이상 회원만 글을 볼 수 있는 게시판입니다.</h4>
        <span id="mem_info">
            <?php
...
if(isset($_SESSION['userid'])){ // 로그인한 사용자인 경우
        echo '<div id="search_box">
            <form action="hard_search_result.php" method="get">
                <select name="catgo">
        ...
        if(isset($_GET['page'])){
         $page = $_GET['page'];
             ...
             $sql2 = "select * from hard order by idx desc limit $start_num, $list";
             ...
             $title=$board["title"]; 
               ...
               $con_idx = $board["idx"];
               $reply_count = "SELECT COUNT(*) as cnt FROM hard_reply where con_num=$con_idx";
            ...
            <tbody>
                <tr>
                    ...
                    if($board['lock_post'] == "1") {
                        ?><a href="hard_ck_read.php?idx=<?php echo $board["idx"];?>"><?php echo $title, $lockimg, $img; 
                     }else {?>
                            <a href="hard_read.php?idx=<?php echo $board["idx"];?>"><?php echo $title,$img;}?></a>
                    </td>
    ...
    <?php }else{
			echo "<div id='not_use'>열심등급만 볼 수 있는 게시판입니다.<br />글을 작성해서 2포인트를 적립하세요.(열심등급 2포인트)</div>";
		}?>
    </div>
</body>
